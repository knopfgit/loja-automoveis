...
const slugify = (s) => s
    ...
    .replace(/\s+/g, '-')
    .replace(/-+/g, '-');
const code = (p = 'VEI') => `${p}-${(0, crypto_1.randomBytes)(3).toString('hex').toUpperCase()}`;
...
async function computeDre(vehicleId) {
...
async function main() {
    ...
    const roles = await Promise.all(['ADMIN', 'SELLER', 'CUSTOMER'].map((name) => prisma.role.create({ data: { name, isSystem: true } })));
    console.log('💰 Regras de comissão...');
    const ruleSale = await prisma.commissionRule.create({
        data: {
            name: 'Padrão 3% sobre venda',
            type: client_1.CommissionRuleType.PERCENT_SALE,
            percentage: 3,
            isDefault: true,
    ...
    await prisma.commissionRule.create({
        data: {
            name: '20% sobre lucro',
            type: client_1.CommissionRuleType.PERCENT_PROFIT,
            percentage: 20,
        },
    });
    await prisma.commissionRule.create({
        data: {
            ...
            type: client_1.CommissionRuleType.PROGRESSIVE,
            tiers: [
    ...
    for (const [c, name, hasExpiry] of vehicleDocs) {
        const t = await prisma.documentType.create({
            data: { code: c, name, ownerType: client_1.DocumentOwnerType.VEHICLE, hasExpiry },
        });
        docTypes[c] = t.id;
    }
    for (const [c, name, hasExpiry] of personDocs) {
        const t = await prisma.documentType.create({
            data: { code: c, name, ownerType: client_1.DocumentOwnerType.BUYER, hasExpiry },
        });
        docTypes[c] = t.id;
    }
    await prisma.documentChecklist.createMany({
        data: [
            { stage: client_1.ChecklistStage.PURCHASE, documentTypeId: docTypes['NF_COMPRA'], position: 1 },
            { stage: client_1.ChecklistStage.PURCHASE, documentTypeId: docTypes['LAUDO'], position: 2 },
            { stage: client_1.ChecklistStage.STOCK_ENTRY, documentTypeId: docTypes['CRLV'], position: 1 },
            { stage: client_1.ChecklistStage.STOCK_ENTRY, documentTypeId: docTypes['VISTORIA'], position: 2 },
            { stage: client_1.ChecklistStage.SALE, documentTypeId: docTypes['CONTRATO_VENDA'], position: 1 },
            { stage: client_1.ChecklistStage.SALE, documentTypeId: docTypes['RG'], position: 2 },
            { stage: client_1.ChecklistStage.SALE, documentTypeId: docTypes['CPF'], position: 3 },
            { stage: client_1.ChecklistStage.SALE, documentTypeId: docTypes['COMP_RES'], position: 4 },
            { stage: client_1.ChecklistStage.TRANSFER, documentTypeId: docTypes['ATPV'], position: 1 },
        ],
    });
    console.log('🚗 Veículos...');
    const vehiclesSeed = [
        { brand: 'Volkswagen', model: 'T-Cross', version: '200 TSI Comfortline', my: 2023, mileage: 32000, purchase: 95000, announced: 119900, status: client_1.VehicleStatus.AVAILABLE, fuel: client_1.FuelType.FLEX, transmission: client_1.Transmission.AUTOMATIC, color: 'Prata', category: 'SUV', featured: true },
        { brand: 'Chevrolet', model: 'Onix', version: '1.0 Turbo Premier', my: 2022, mileage: 41000, purchase: 68000, announced: 84900, status: client_1.VehicleStatus.AVAILABLE, fuel: client_1.FuelType.FLEX, transmission: client_1.Transmission.AUTOMATIC, color: 'Branco', category: 'Hatch', featured: true },
        { brand: 'Toyota', model: 'Corolla', version: '2.0 XEI', my: 2021, mileage: 55000, purchase: 105000, announced: 129900, status: client_1.VehicleStatus.AVAILABLE, fuel: client_1.FuelType.FLEX, transmission: client_1.Transmission.CVT, color: 'Preto', category: 'Sedan' },
        { brand: 'Fiat', model: 'Toro', version: '2.0 Diesel Volcano', my: 2022, mileage: 38000, purchase: 130000, announced: 159900, status: client_1.VehicleStatus.AVAILABLE, fuel: client_1.FuelType.DIESEL, transmission: client_1.Transmission.AUTOMATIC, color: 'Cinza', category: 'Picape', featured: true },
        { brand: 'Honda', model: 'HR-V', version: '1.8 EXL', my: 2020, mileage: 62000, purchase: 92000, announced: 112900, status: client_1.VehicleStatus.AVAILABLE, fuel: client_1.FuelType.FLEX, transmission: client_1.Transmission.CVT, color: 'Vermelho', category: 'SUV' },
        { brand: 'Hyundai', model: 'Creta', version: '1.0 Turbo Comfort', my: 2023, mileage: 21000, purchase: 98000, announced: 124900, status: client_1.VehicleStatus.RESERVED, fuel: client_1.FuelType.FLEX, transmission: client_1.Transmission.AUTOMATIC, color: 'Azul', category: 'SUV' },
        { brand: 'Jeep', model: 'Compass', version: '1.3 Turbo Limited', my: 2022, mileage: 45000, purchase: 125000, announced: 154900, status: client_1.VehicleStatus.IN_MAINTENANCE, fuel: client_1.FuelType.FLEX, transmission: client_1.Transmission.AUTOMATIC, color: 'Branco', category: 'SUV' },
        { brand: 'Ford', model: 'Ranger', version: '3.2 XLT Diesel 4x4', my: 2021, mileage: 70000, purchase: 145000, announced: 179900, status: client_1.VehicleStatus.SOLD, fuel: client_1.FuelType.DIESEL, transmission: client_1.Transmission.AUTOMATIC, color: 'Prata', category: 'Picape' },
        { brand: 'Volkswagen', model: 'Gol', version: '1.6 MSI Comfortline', my: 2020, mileage: 58000, purchase: 48000, announced: 59900, status: client_1.VehicleStatus.SOLD, fuel: client_1.FuelType.FLEX, transmission: client_1.Transmission.MANUAL, color: 'Branco', category: 'Hatch' },
        { brand: 'Chevrolet', model: 'Tracker', version: '1.2 Turbo Premier', my: 2023, mileage: 18000, purchase: 108000, announced: 134900, status: client_1.VehicleStatus.AVAILABLE, fuel: client_1.FuelType.FLEX, transmission: client_1.Transmission.AUTOMATIC, color: 'Cinza', category: 'SUV', featured: true },
        { brand: 'Hyundai', model: 'HB20', version: '1.0 Sense', my: 2021, mileage: 49000, purchase: 52000, announced: 66900, status: client_1.VehicleStatus.DRAFT, fuel: client_1.FuelType.FLEX, transmission: client_1.Transmission.MANUAL, color: 'Prata', category: 'Hatch' },
    ];
    const vehicles = [];
    ...
    const maintVehicle = vehicles.find((v) => v._seed.status === client_1.VehicleStatus.IN_MAINTENANCE);
    ...
    const reservedVehicle = vehicles.find((v) => v._seed.status === client_1.VehicleStatus.RESERVED);
    ...
    const soldVehicles = vehicles.filter((v) => v._seed.status === client_1.VehicleStatus.SOLD);
